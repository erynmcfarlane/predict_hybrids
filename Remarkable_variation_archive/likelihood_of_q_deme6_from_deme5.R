...
setwd("~/Google Drive/Replicate Hybrid zone review/Predicting_Hybrids_analysis/hybrid_sims/deme11_all")
...
for(i in 1:length(datafiles)){
  ...
  alldata[[i]]$mech<-rep(mech[i], nrow(alldata[[i]]))
}
...
alldata_df$q<-ifelse(alldata_df$q ==0, alldata_df$q+0.001, ifelse(alldata_df$q == 1, alldata_df$q-0.001, alldata_df$q))

get_dist<-function(x){
  estimates<-fitdistr(x, 'beta', start=list(shape1=10,shape2=10), lower=c(0,0), upper=c(1, 1), method="CG") ### not at all sure how to decide on the starting shapes!
  estimates$estimate
...
for(j in 1:20){ ##change this back to 20
  beta_parameters<-with(alldata_df[c(alldata_df$rep!=j & alldata_df$deme==5),], aggregate(q, list(m, c, gen, mech), get_dist))
  ...
  alldata_df$index<-paste(alldata_df$m, alldata_df$c, alldata_df$gen)
